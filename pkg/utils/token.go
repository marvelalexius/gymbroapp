...
type TokenClaims struct {
	...
	User       *response.AuthResponse `json:"user"`
...
func GenerateToken(user *response.AuthResponse, lifespan int, duration string, secret string) (*Token, error) {
	token := &Token{}
	expTime := time.Time{}
...
func ComparePassword(hashed string, plain string) (bool, error) {
	...
	if err != nil {
		return false, errors.New("your current password is wrong, please try again")
	}
